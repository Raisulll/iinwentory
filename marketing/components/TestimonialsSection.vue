    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="mx-auto max-w-2xl text-center reveal">
        ...
        <h2 class="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl">
          Used every day by operators<br />who count for a living
        </h2>
      ...
      <div class="mt-12 sm:mt-16 grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <div
          v-for="(t, i) in testimonials"
          :key="t.name"
          :class="['relative rounded-2xl bg-white border border-gray-100 p-5 sm:p-6 shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 break-words reveal', `reveal-delay-${(i % 3) + 1}`]"
        >
          <!-- Stars -->
          <div class="flex gap-0.5 mb-4">
            <svg v-for="i in t.rating" :key="i" class="size-4 text-amber-400 fill-current" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
          </div>

          <blockquote class="text-sm text-gray-600 leading-relaxed break-words">
            "{{ t.quote }}"
          </blockquote>

          <div class="mt-5 flex items-center gap-3">
            <div class="flex size-9 sm:size-10 items-center justify-center rounded-full bg-primary-100 text-xs sm:text-sm font-bold text-primary-700 shrink-0">
              {{ t.avatar }}
            </div>
            <div class="flex-1 min-w-0">
              <div class="text-sm font-semibold text-gray-900 truncate">{{ t.name }}</div>
              <div class="text-xs text-gray-500 truncate">{{ t.role }}, {{ t.company }}</div>
            </div>
            <span class="rounded-full px-2 py-0.5 text-[10px] sm:text-xs font-semibold shrink-0" :class="planColors[t.plan]">
              {{ t.plan }}
            </span>
          </div>
        </div>
