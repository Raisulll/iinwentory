...
<template>
  <section id="features" class="bg-bg py-24 lg:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      ...
      <div class="mx-auto max-w-2xl text-center reveal">
        ...
        <h2 class="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl">
          Every feature your<br />inventory actually needs
        </h2>
      ...
      <div class="mt-20 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <div
          ...
          :key="feature.title"
          :class="['group relative rounded-2xl bg-white p-6 border border-gray-100 shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 reveal', `reveal-delay-${(i % 4) + 1}`]"
        >
          <div :class="`mb-4 inline-flex size-12 items-center justify-center rounded-2xl bg-gradient-to-br ${feature.color} text-2xl shadow-lg ring-1 ring-white/20 transition-transform group-hover:scale-110 group-hover:rotate-3`">
            {{ feature.icon }}
          </div>
          <h3 class="text-base font-bold text-gray-900 tracking-tight">{{ feature.title }}</h3>
          <p class="mt-2 text-sm text-gray-500 leading-relaxed">{{ feature.description }}</p>
          <ul class="mt-4 space-y-1.5 pt-4 border-t border-gray-100">
            <li
              v-for="h in feature.highlights"
              :key="h"
              class="flex items-center gap-2 text-xs font-medium text-gray-600"
            >
              <svg class="size-3.5 shrink-0 text-emerald-500" fill="currentColor" viewBox="0 0 20 20">
